...
def create_tables():
    ...
    for query in queries:
        execute_query(query)
def fill_archive_table():
    year = datetime.now().year
    archive_data_queries = [
        """
        INSERT INTO Archive (Passenger_ID, Luggage_ID, Service_ID, Flight_ID, Year)
        SELECT p.Aadhar_number, l.Luggage_id, a.Service_id, pf.Flight_number, ?
        FROM Passenger p
        LEFT JOIN Luggage l ON p.Aadhar_number = l.Aadhar_number
        LEFT JOIN Passenger_Flight pf ON p.Aadhar_number = pf.Aadhar_number
        LEFT JOIN Admin a ON a.Service_id = (SELECT Service_id FROM Terminal_Service ORDER BY RANDOM() LIMIT 1)
        """
    ]
    
    for query in archive_data_queries:
        execute_query(query, (year,))
class AirportManagementSystem:
    def __init__(self, master):
        self.master = master
        self.master.title("Airport Passenger Management System")
    ...
    def create_main_menu(self):
        ...
        tk.Label(self.master, text="Airport Passenger Management System", font=("Arial", 24)).pack(pady=20)
    ...
    def update_status(self, status_type, id_value, new_status, id_column, table):
        ...
        if status_type == "flight":
            # Update or insert into flight_status table
            check_query = "SELECT * FROM flight_status WHERE flight_number = ?"
            result = execute_query(check_query, (id_value,))
            if result:
                update_query = "UPDATE flight_status SET Status = ? WHERE flight_number = ?"
            else:
                update_query = "INSERT INTO flight_status (Status, flight_number) VALUES (?, ?)"
            execute_query(update_query, (new_status, id_value))

        if status_type == "luggage":
            # Update or insert into luggage_status table
            check_query = "SELECT * FROM luggage_status WHERE luggage_number = ?"
            result = execute_query(check_query, (id_value,))
            if result:
                update_query = "UPDATE luggage_status SET Status = ? WHERE luggage_number = ?"
            else:
                update_query = "INSERT INTO luggage_status (Status, luggage_number) VALUES (?, ?)"
            execute_query(update_query, (new_status, id_value))
    ...
    def track_luggage_by_id(self, luggage_id):
        ...
        """
        query1="""
        SELECT Status FROM luggage_status WHERE luggage_number=?
        """
        
        result = execute_query(query, (luggage_id,))
        result1=execute_query(query1, (luggage_id,))
        ...
        if result:
            ...
            tk.Label(self.master, text=f"Status: {result1}").pack()
        else:
